...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run exp11 across pure_greedy, top_k, top_p, dola, always_contrast, and panda_switch."
    )
...
def main():
    ...
    for row in selected_rows:
        ...
        print(
            {
                ...
                "decoder_names": list(EXP11_DECODER_NAMES),
            }
        )
        if args.dry_run:
            continue
        ...
        evaluator = ExperimentEvaluator(evaluator_args, EXP11_DECODER_NAMES)
        metadata = {
            ...
            "decoder_names": list(EXP11_DECODER_NAMES),
            "hypothesis": (
                "always_contrast_should_remain_the_strongest_accuracy_baseline_unless_original_"
                "panda_switch_recovers_value_over_classic_truncation_and_dola_families"
            ),
            "experiment_note": (
                "pure_greedy_vs_topk_topp_vs_dola_vs_always_contrast_vs_original_panda_switch"
            ),
